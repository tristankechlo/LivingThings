...
public class AncientBlazeConfig {
	...
	public AncientBlazeConfig(ForgeConfigSpec.Builder builder) {
		...
		health = builder.comment(LivingThingsConfig.REQUIRES_RESTART).worldRestart().defineInRange("Health", 250.0D,
		...
		chargingTime = builder
				.comment("time in ticks, until the entity can be damaged | " + LivingThingsConfig.REQUIRES_RESTART)
				.worldRestart().defineInRange("InvulnerableTime", 200, 10, Short.MAX_VALUE);

		blazeSpawnCount = builder.comment("amount of blazes spawned on death | " + LivingThingsConfig.REQUIRES_RESTART)
				.worldRestart().defineInRange("BlazeSpawnCount", 4, 0, 16);

		largeFireballAmount = builder
				.comment("max amount of large fireballs to shoot | " + LivingThingsConfig.REQUIRES_RESTART)
				.worldRestart().defineInRange("LargeFireballCount", 6, 0, 10);

		largeFireballChance = builder
				.comment("chance for the attack to be a large fireball | " + LivingThingsConfig.REQUIRES_RESTART)
				.worldRestart().defineInRange("LargeFireballChance", 20, 0, 100);

		canAttack = builder.define("CanAttack", true);

		peacefulDespawn = builder.comment("wether or not the entity shall despawn when peaceful is turned on")
				.define("PeacefulDespawn", true);
